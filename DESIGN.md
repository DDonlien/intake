...
- `reference/ts/` 原型使用固定手机画布：`402px × 874px`
- 桌面浏览器中外层只作为预览舞台，手机画布居中展示
- 页面内容在手机画布内部纵向滚动，底部 Tab Bar 固定在画布底部
- 原型用于验证 iOS App 信息结构、交互节奏和视觉方向，不作为 Web 产品设计规范
- Web 端无法实现 iOS 26 原生 `.glassEffect()` 的真实反射/折射，只能用 CSS 近似表达材质层级
