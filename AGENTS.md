...
- 一次任务执行从 Agent 开始处理用户请求算起，到交付、提交、停止或明确暂停为止。
...
- 项目名称：Intake
- 产品简介：AI-powered nutrition budget for everyday meals. 结合 Apple Health 实时消耗、营养数据库和 AI 拍照识别的 iOS 饮食预算 App。
- 主要用户：关注每日热量预算和蛋白质目标的 iOS 用户
- 当前阶段：MVP 界面实现阶段

### 技术栈与命令

- 技术栈：SwiftUI, iOS 17+, HealthKit, SwiftData
- 开发命令：Xcode 打开 `Intake.xcodeproj` 或使用 `swift` 命令
- 测试命令：Cmd+U in Xcode
- 构建命令：Cmd+B in Xcode
- 发布命令：Xcode Archive & Distribute
...
- 根目录：项目文档、Xcode 项目文件
- 子功能目录：`Intake/` 主应用源代码
- 资源目录：`Intake/Assets.xcassets/`, `reference/`
- 文档目录：`agent-log/`, `reference/`
- 测试目录：`IntakeTests/`, `IntakeUITests/`
...
- 语言与命名：Swift 代码使用英文命名；UI 文案优先英文；中文文案根据需求添加
- 设计原则：iOS 原生 Liquid Glass 风格；默认浅色模式；深色模式可扩展；信息密度适中；避免卡通减肥 App 风格
- 架构限制：iOS 17+；SwiftUI 优先；使用系统原生组件和 API
- 授权与引用边界：Apple Health 数据属于敏感数据；照片/语音数据需明确隐私说明；AI 功能作为付费边界
- 安全、隐私或合规要求：遵循 App Store HealthKit 审核指南；遵循 Apple 隐私政策要求；内购需审核备注说明
